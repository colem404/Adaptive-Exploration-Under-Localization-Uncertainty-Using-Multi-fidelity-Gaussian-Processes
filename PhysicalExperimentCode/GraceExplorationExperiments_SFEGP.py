...
def replan():#(agent,df1,df2,df3,planNum,GPpreTrained=None):		
	global edgeChain,pathPoints,planner,totalTime,allPathPoints
	#load saved data
	print("planning")
	...
	if ess.updateGPHyps:
		...
		try:
			gp.optimize()
			gp.param_array[gp.param_array>90]=1
			HypVec=gp.param_array.copy()
		except Exception as e:
			print(e)															
	gp._save_model(savePath+"GPmodel{0}".format(planNum),compress=False)
	f=open(savePath+"GPySFGP.csv","a")
	...
	np.savetxt(savePath+"EID{0}.csv".format(planNum),np.concatenate((agent.fieldGrid,agent.EID),axis=1),delimiter=",",header=" x,y,z,EID",comments="")
	agent.sfgp=gp
...
ys=ess.ftf(measVars[np.isin(measVars[:,0],df1+df2+df3),-1])
ys.shape=(ys.shape[0],1)
...
gpHyps=ess.initHyps
#kernel=GPy.kern.RBF(input_dim=3,variance=gpHyps[0],lengthscale=gpHyps[1:4],ARD=True)
kernel=GPy.kern.Matern32(input_dim=3,variance=gpHyps[0],lengthscale=gpHyps[1:4],ARD=True)
gp=GPy.models.GPRegression(Xhs,ys,kernel)
gp.likelihood.variance=gpHyps[4]
gp._save_model(savePath+"GPmodel{0}".format(planNum),compress=False)
HypNames=gp.parameter_names()
f=open(savePath+"GPySFGP.csv","w")
...
EID,_=ess.getEID(gp,WS,ess.maxDepth,testSet=testPoints)
np.savetxt(savePath+"resultsSF.csv",np.concatenate((testPoints,mu,sig,EID),axis=1),delimiter=",",header=" x,y,z,gpMean,gpVar,EID",comments="")
