...
class GraceAgent():
	...
	def calcPathErgodicity(self,V,E,path,dense=False):
		pnts=self.pathToTrajPoints(V,E,path,dense=dense)
		X=pnts[:,:3]
		t=pnts[:,3:4]
...
class RIG:
	...
	def __init__(self,delta,B,WS,FS,Env,R,agent=None):
		...
		self.bestBudgetPath=None
		self.bestInfoPath=None
		self.worstBudgetPath=None
		self.worstInfoPath=None
		self.maxIter = 20
		self.curIter = 0
...
class Graph(RIG):
						...
						if pathBudget<self.B:
							...
							if not self.ModularCost :
								...
								if len(self.V)>1 and pathBudget>self.budgetCutoff*self.B:
									...
									#if self.bestBudgetPath == None:
									#	self.bestBudgetPath=(pathBudget,pathInfo)
									#	self.bestInfoPath=(pathInfo,pathBudget)
									#	self.worstBudgetPath=(pathBudget,pathInfo)
									#	self.worstInfoPath=(pathInfo,pathBudget)
									#else:
									#	self.bestBudgetPath= max(self.bestBudgetPath,(pathBudget,pathInfo))
									#	self.bestInfoPath= max(self.bestInfoPath,(pathInfo,pathBudget))
									#	self.worstBudgetPath=min(self.worstBudgetPath,(pathBudget,pathInfo))
									#	self.worstInfoPath=min(self.worstInfoPath,(pathInfo,pathBudget))
								else:
									pathInfo=-10000
	...
	def plan(self,xstart,R=None,Rd=0):#plan from scratch
		...
		self.Vc=Vc
		self.Vidx=Vidx
		#print(self.bestInfoPath,self.worstInfoPath,self.bestBudgetPath,self.worstBudgetPath)
		#self.search()
		#self.childlessNodes()
